...
# Get dbHomeId
# ./oci-curl.sh database.us-ashburn-1.oraclecloud.com get "/20160918/dbHomes?compartmentId=$compartmentId&dbSystemId=$dbSystemId&limit=10"
...
if [[ $# -lt 3 ]]; then
  echo "Usage: source ./env.sh; ./oci-curl.sh <host> <method> [file-to-send-as-body] <request-target> [extra-curl-args]"
  echo "Example: source ./env.sh; ./oci-curl.sh iaas.us-ashburn-1.oraclecloud.com get /20160918/instances?compartmentId=some-compartment-ocid"
  echo "Example: source ./env.sh; ./oci-curl.sh iaas.us-ashburn-1.oraclecloud.com post ./request.json /20160918/vcns"
  exit 2
fi
